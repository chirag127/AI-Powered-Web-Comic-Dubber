...
class TextExtractor {
  ...
  async extractText(image, bubble) {
    ...
    ctx.drawImage(image, bubble.x, bubble.y, bubble.width, bubble.height, 0, 0, bubble.width, bubble.height);
...
function updateTextCorrectionUI(detectedTexts) {
  ...
  const textCorrectionDiv = document.getElementById('comic-dubber-text-correction');
...
async function detectBubbles() {
  if (!checkComponentsLoaded() || comicImages.length === 0) {
    return { success: false, error: 'Components not loaded or no comic images found' };
  }
...
window.addEventListener('load', () => {
  ...
  // Check periodically if components are loaded
